...
public class TGNavigationController: UINavigationController {
    ...
    public override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if self.viewControllers.isEmpty == false {
            let backBarItem = UIBarButtonItem(image: UIImage.set_image(named: "btn_back_normal")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(popBack))
