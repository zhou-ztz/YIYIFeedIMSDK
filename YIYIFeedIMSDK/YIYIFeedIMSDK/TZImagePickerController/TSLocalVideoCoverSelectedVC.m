...
- (void)creatNavUI {
    self.navigationItem.title = [NSBundle tz_localizedStringForKey:@"select_cover_photo"];

    UIButton *rightButton = [UIButton buttonWithType:UIButtonTypeCustom];
    rightButton.frame = CGRectMake(0, 0, 44, 44);
    rightButton.titleLabel.font = [UIFont systemFontOfSize:16];
    [rightButton setTitle: [NSBundle tz_localizedStringForKey:@"Done"] forState:UIControlStateNormal];
    if (_mainColor) {
...
- (void)rightButtonClick {
    ...
    if (self.coverImageBlock) {
        if (self.centerImageView.image == nil) {
            ...
            UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:[NSBundle tz_localizedStringForKey:@"Alert"] message:[NSBundle tz_localizedStringForKey:@"cover_photo_unusable"] preferredStyle:UIAlertControllerStyleAlert];
