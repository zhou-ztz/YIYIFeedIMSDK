...
/// if PHAuthorizationStatusLimited 返回YES 相册权限受限
- (BOOL)authorizationStatusLimited {
    if (self.isPreviewNetworkImage) {
        return YES;
    }
    ...
    if (@available(iOS 14, *)) {
        ...
        status = [PHPhotoLibrary authorizationStatus]; // iOS 14 以下无 `authorizationStatusForAccessLevel`
    }
...
- (void)getVideoOutputPathWithAsset:(PHAsset *)asset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure progressUpdate:(SEL)progressUpdate inClass:(id)inClass {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset* avasset, AVAudioMix* audioMix, NSDictionary* info){
        ...
        AVURLAsset *videoAsset = (AVURLAsset*)avasset;
        // NSLog(@"AVAsset URL: %@",myAsset.URL);
        [self startExportVideoWithVideoAsset:videoAsset presetName:presetName success:success failure:failure progressUpdate:progressUpdate inClass:inClass];
    }];
}
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        AVAssetTrack * originalAssetVideoTracks = [[videoAsset tracksWithMediaType:AVMediaTypeVideo] firstObject];
        
        AVMutableComposition *mixComposition = [[AVMutableComposition alloc] init];
        AVMutableCompositionTrack *videoTrack = [mixComposition addMutableTrackWithMediaType:AVMediaTypeVideo preferredTrackID:1];
        [videoTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, videoAsset.duration) ofTrack:originalAssetVideoTracks atTime:kCMTimeZero error:nil];
        ...
        if ([[videoAsset tracksWithMediaType:AVMediaTypeAudio] count]>0) {
            ...
            [firstAudioTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, videoAsset.duration) ofTrack:[[videoAsset tracksWithMediaType:AVMediaTypeAudio] objectAtIndex:0] atTime:kCMTimeZero error:nil];
        }else{
            NSLog(@"warning: video has no audio");
        }
        
        AVAssetExportSession *session = [[AVAssetExportSession alloc] initWithAsset:mixComposition presetName:presetName];
        NSDateFormatter *formater = [[NSDateFormatter alloc] init];
        [formater setDateFormat:@"yyyy-MM-dd-HH:mm:ss-SSS"];
        NSString *outputPath = [NSHomeDirectory() stringByAppendingFormat:@"/tmp/video-%@.mp4", [formater stringFromDate:[NSDate date]]];
        ...
        if ([TZImagePickerConfig sharedInstance].needFixComposition) {
            AVMutableVideoComposition *videoComposition = [self fixedCompositionWithAsset:videoAsset assetVideoTrack:videoTrack];
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure progressUpdate:(SEL)progressUpdate inClass:(id)inClass {
    // Find compatible presets by video asset.
    NSArray *presets = [AVAssetExportSession exportPresetsCompatibleWithAsset:videoAsset];
    ...
    // If you need to upload to the server, but server does't support to upload by streaming,
    // You can compress the resolution to lower. Or you can support more higher resolution.
    if ([presets containsObject:presetName] && [[videoAsset tracksWithMediaType:AVMediaTypeVideo] count] > 0) {
        AVAssetTrack * originalAssetVideoTracks = [[videoAsset tracksWithMediaType:AVMediaTypeVideo] firstObject];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            exportTimer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:inClass selector:progressUpdate userInfo:session repeats:TRUE];
            [[NSRunLoop currentRunLoop] addTimer:exportTimer forMode:NSRunLoopCommonModes];
        });
    } else {
        if (failure) {
            NSString *errorMessage = [NSString stringWithFormat:@"当前设备不支持该预设:%@", presetName];
...
/// 获取优化后的视频转向信息
- (AVMutableVideoComposition *)fixedCompositionWithAsset:(AVAsset *)videoAsset assetVideoTrack:(AVAssetTrack *)videoTrack {
