    ...
    @objc func pinchGesture(_ recognizer: UIPinchGestureRecognizer) {
        if let view = recognizer.view as? UILabel {
            switch recognizer.state {
                case .changed:
                    let scale = sqrt(Double(view.transform.a * view.transform.a + view.transform.c * view.transform.c))
                    if ((scale > 8 && recognizer.scale > 1) || (scale < 0.6 && recognizer.scale < 1)) == false {
                        view.transform = view.transform.scaledBy(x: recognizer.scale, y: recognizer.scale)
                        view.contentScaleFactor = 7
                    }
                    recognizer.scale = 1
                    break
