...
class VideoCallingCell: BaseMessageCell {
    ...
    override func setData(model: TGMessageData) {
        ...
        guard let message = model.nimMessageModel, let dict = model.customDict , let data = dict.object(forKey: CMData) as? NSDictionary, let type = Int(data.jsonString(CMCallType)), let callType = V2NIMSignallingChannelType(rawValue: type), let eventType = Int(data.jsonString(CMEventType)), let callEventType = CallingEventType(rawValue: eventType) else { return }
        let duration = Double(data.jsonString(CMCallDuration)) ?? 0
        var image = ""
        var text = "unknown_message".localized
        if callType == .SIGNALLING_CHANNEL_TYPE_VIDEO {
            image = "icon_video_call"
        }else {
            image = "icon_voice_call"
        }
        callImageView.image = UIImage(named:image)
        text = netcallFormatedMessage(callEventType, duration: duration)
        infoLabel.text = text
    }
