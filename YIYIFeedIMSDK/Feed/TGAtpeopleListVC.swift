...
class TGAtpeopleListVC: TGViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, TGMyFriendListCellDelegate {
    ...
    @objc func refresh() {
        if !isSearching {
            TGNewFriendsNetworkManager.searchMyFriend(offset: nil, keyWordString: keyword, filterMerchants: "1") {[weak self] users, error in
    ...
    @objc func loadMore() {
        TGNewFriendsNetworkManager.searchMyFriend(offset: dataSource.count, keyWordString: keyword, filterMerchants: "1", completion: { (userModels, networkError) in
...
//            friendListTableView.show(placeholderView: .network)
    ...
    // MARK: - TGMyFriendListCellDelegate
