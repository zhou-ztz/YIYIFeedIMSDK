...
- (void)viewDidLoad {
    ...
    [self setDefaultData];
    [self createUI];
    self.titleLabel.text = [NSBundle tz_localizedStringForKey:@"crop_image"];
}
...
-(void)viewDidLayoutSubviews{
    ...
    }else if ([self lz_isIPhone14AndAbove]) {
        [self.navView setFrame:CGRectMake(0, 0, _selfWidth, isStatusBarHidden ? 44 : 64 +24)];
        self.titleLabel.frame = CGRectMake(0, 0, 200, 20);
        self.titleLabel.center = CGPointMake(self.navView.frame.size.width / 2.0, (self.navView.frame.size.height - (isStatusBarHidden ? 0 : self.titleLabel.frame.size.height)) / 2.0 + (isStatusBarHidden ? 0 : 20 + 18));
...
-(UIButton *)sureButton{
    if (!_sureButton) {
        ...
        [_sureButton setTitle:[NSBundle tz_localizedStringForKey:@"Done"] forState:UIControlStateNormal];
...
- (BOOL)lz_isIPhone14AndAbove {
    struct utsname systemInfo;
    uname(&systemInfo);
    NSString *platform = [NSString stringWithCString:systemInfo.machine encoding:NSASCIIStringEncoding];
    BOOL isIPhone14AndAbove = [platform containsString:@"iPhone14"] || [platform containsString:@"iPhone15"]
    || [platform containsString:@"iPhone16"];
    
    return isIPhone14AndAbove;
}
